...
/**
 * Created by Quang Le on 2/12/18.
 *
...
class Node7(name: String) {
  val name = name
  var weight: Int = 0
  var parent: Node7? = null
  var children: MutableList<Node7> = ArrayList()

  constructor(name: String, weight: Int) : this(name) {
    this.weight = weight
  }

  fun isBasedNode(): Boolean {
    return children.size != 0
  }

  fun addChild(child: Node7) {
    children.add(child)
    child.parent = this
  }

  fun containNode(childName: String): Boolean {
    return isBasedNode()
        && children.any { it -> it.name.equals(childName) || it.containNode(childName) }
  }

  fun updateChildTree(child: Node7): Boolean {
    if (!isBasedNode()) {
      return false
    }

    if (children.any { it -> it.name.equals(child.name) }) {
      children.removeIf { it.name.equals(child.name) }
      addChild(child)
      return true
    }

    return children.any { it.updateChildTree(child) }
  }

  fun computeTreeWeight(): Int {
    weight = weightMap[name]!! + children.sumBy { it ->
      it.computeTreeWeight()
    }
    return weight
  }

  fun getUnbalanceTree(): Node7 {
    val childWeightMap = children.groupBy { it -> it.weight }
    return if (childWeightMap.size < 2) {
      this
    } else {
      childWeightMap.values.find { listNode -> listNode.size == 1 }!!.first().getUnbalanceTree()
    }
  }
}

var rootNode: Node7 = Node7("")
val weightMap = HashMap<String, Int>()

fun day7a(): String {
  val nodes = ArrayList<Node7>()

  StringReader(DAY_7_INPUT).readLines().onEach {
    val node = parseNode(it)
    weightMap.put(node.name, node.weight)
    nodes.add(node)
  }

  val ite = nodes.iterator()
  while (ite.hasNext()) {
    val node = ite.next()
    for (baseNode in nodes) {
      if (baseNode.containNode(node.name)) {
        baseNode.updateChildTree(node)
        ite.remove()
        break
      }
    }
...
fun day7b(): Int {
  day7a()
  rootNode.computeTreeWeight()
  val unbalancedNode = rootNode.getUnbalanceTree()
  val parentOfUnbalanced = unbalancedNode.parent!!
  val balanceTreeWeight = parentOfUnbalanced.children.first { it -> it.weight != unbalancedNode.weight }.weight
  return weightMap[unbalancedNode.name]!! - (unbalancedNode.weight - balanceTreeWeight)
}

fun parseNode(nodeText: String): Node7 {
  val elements = nodeText.split(CHILD_SIGN)
  val nodeName = elements[0].split(" ")[0]
  val nodeWeight = elements[0].split(" ")[1].removeSurrounding("(", ")").toInt()

  val node = Node7(nodeName, nodeWeight)

  if (elements.size > 1) {
    elements[1].split(", ").onEach {
      val childNode = Node7(it)
