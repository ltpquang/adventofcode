...
/**
 * Created by Quang Le on 2/28/18.
 *
...
class Instruction(rawString: String) {
  val registerName: String
  val mode: Mode
  val value: Int
  val registerNameToCheck: String
  val conditionValueToCheck: Int
  val comparingMode: Compare

  init {
    val tokens = rawString.split(" ")
    registerName = tokens[0]
    mode = if (tokens[1].equals(INC)) Mode.INC else Mode.DEC
    value = Integer.parseInt(tokens[2])
    registerNameToCheck = tokens[4]
    conditionValueToCheck = Integer.parseInt(tokens[6])
    comparingMode = when (tokens[5]) {
      "==" -> Compare.EQUAL
      "!=" -> Compare.NOT_EQUAL
...
fun day8a(): Int {
  StringReader(DAY_8_INPUT).readLines().onEach {
    val instruction = Instruction(it)
    val minus = getValue(instruction.registerNameToCheck) - instruction.conditionValueToCheck
    if ((instruction.comparingMode == Compare.EQUAL && minus == 0)
        || (instruction.comparingMode == Compare.NOT_EQUAL && minus != 0)
        || (instruction.comparingMode == Compare.GREATER_THAN && minus > 0)
        || (instruction.comparingMode == Compare.GREATER_THAN_OR_EQUAL && minus >= 0)
        || (instruction.comparingMode == Compare.LESS_THAN && minus < 0)
        || (instruction.comparingMode == Compare.LESS_THAN_OR_EQUAL && minus <= 0)) {
      val oldValue = getValue(instruction.registerName)
      val newValue = if (instruction.mode == Mode.INC) oldValue + instruction.value else oldValue - instruction.value
      setValue(instruction.registerName, newValue)
    }
  }

  return valueMap.values.max()!!
}
...
fun day8b(): Int {
  ...
  StringReader(DAY_8_INPUT).readLines().onEach {
    ...
    if ((instruction.comparingMode == Compare.EQUAL && minus == 0)
      ...
      val newValue = if (instruction.mode == Mode.INC) oldValue + instruction.value else oldValue - instruction.value
      setValue(instruction.registerName, newValue)
      if (getValue(instruction.registerName) > max) {
        max = getValue(instruction.registerName)
      }
    }
  }
